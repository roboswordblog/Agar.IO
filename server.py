        ...
        while True:
            ...
            with lock:
                ...
                if "actualX" in msg and "actualY" in msg:
                    ...
                    collisionid = msg.get("id")

                    if collisionid is not None:
                        collisionid = str(collisionid)  # IMPORTANT: match dict keys

                        if collisionid in players:
                            print("REMOVING", collisionid)

                            try:
                                connections[collisionid].send((json.dumps({"dead": True}) + "\n").encode())
                            except:
                                pass

                            players.pop(collisionid)
                            connections.pop(collisionid, None)
